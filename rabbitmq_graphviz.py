...
def build_definitions(definitions, vhost, render_producers, render_consumers):
    ...
    def is_same_vhost(entity_dict):
        return entity_dict.get('vhost', '/') == vhost
    ...
    return ''.join([
        ...
        '  rankdir=LR;\n',
        '  node [style="filled"];\n\n',
        ''.join([build_queue(q, render_consumers)
                for q in filter(is_same_vhost, definitions['queues'])]),
        ''.join([build_exchange(x, render_producers)
                for x in filter(is_same_vhost, definitions['exchanges'])]),
        ''.join([build_binding(b)
                for b in filter(is_same_vhost, definitions['bindings'])]),
        ''.join([build_dlx_link(q)
                for q in filter(is_same_vhost, definitions['queues'])]),
        '}'])
...
def build_exchange(exchange, render_producers):
    ...
    if render_producers:
        lines.append(
            '  "P_%s" [label="P", style="filled", fillcolor="#00ffff"];' % x_id)
...
def parse_args():
    ...
    parser.add_argument('-p', '--producers',
                        action='store_true', help='Render producers')
    parser.add_argument('-c', '--consumers',
                        action='store_true', help='Render consumers')
    parser.add_argument('-x', '--vhost', nargs='?',
                        default='/', help='Restrict to this vhost')
...
if __name__ == '__main__':
    ...
    args.outfile.write(build_definitions(
        definitions, args.vhost, args.producers, args.consumers))
    args.outfile.close()
